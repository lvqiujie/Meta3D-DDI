...
import numpy as np
import sys
from utils.storage import build_experiment_folder, save_statistics, save_to_json, sigmoid
...
class ExperimentBuilder(object):
    ...
    def build_loss_summary_string(self, summary_losses):
        ...
        for key, value in zip(list(summary_losses.keys()), list(summary_losses.values())):
            if "loss" in key or "accuracy" in key or "auc" in key or "f1" in key or "precision" in key or "recall" in key or "ap" in key:
                value = float(value)
    ...
    def test_evaluation_iteration(self, val_sample, model_idx, sample_idx, per_model_per_batch_preds, pbar_test):
        ...
        per_model_per_batch_preds[model_idx].extend(per_task_preds)

        test_output_update = self.build_loss_summary_string(losses)

        pbar_test.update(1)
        pbar_test.set_description(
            "test_phase {} -> {}, \n".format(self.epoch, test_output_update))
    ...
    def evaluated_test_set_using_the_best_models(self, top_n_models, test_name="test1"):
        ...
        for idx, model_idx in enumerate(top_n_idx):
            state = self.model.load_model(model_save_dir=self.saved_models_filepath, model_name="train_model",
                                      model_idx=model_idx + 1)
            print("test  :", model_idx + 1, "\n")
            with tqdm.tqdm(total=int(self.args.num_evaluation_tasks / self.args.batch_size)) as pbar_test:
                if "test1".__eq__(test_name):
                    for sample_idx, test_sample in enumerate(self.data.get_test1_batches(total_batches=int(self.args.num_evaluation_tasks / self.args.batch_size))):
                        per_model_per_batch_targets[idx].extend(test_sample[6])
                    ...
                    for sample_idx, test_sample in enumerate(self.data.get_test2_batches(total_batches=int(self.args.num_evaluation_tasks / self.args.batch_size))):
                        per_model_per_batch_targets[idx].extend(test_sample[6])
                        per_model_per_batch_preds = self.test_evaluation_iteration(val_sample=test_sample,
                                                                                   sample_idx=sample_idx,
                                                                                   model_idx=idx,
        ...
        # per_model_per_batch_preds = np.array(per_model_per_batch_preds).reshape(-1)
        ...
        ap_list = []
        for i in range(top_n_models):
            preds_y = sigmoid(np.array(per_model_per_batch_preds[i]))
            if len(preds_y) == 0:
                continue
            targets = []
            for b in range(len(per_model_per_batch_targets[i])):
                for c in range(self.args.num_classes_per_set):
                    targets.extend(per_model_per_batch_targets[i][b][c].numpy())

            accuracy, auroc, f1_score, precision, recall, ap = do_compute_metrics(preds_y,
                                                                                  targets)
            acc.append(accuracy)
            ...
            ap_list.append(ap)
        print(test_name, "acc mean", np.mean(acc), "acc std", np.std(acc),
              "auc mean", np.mean(auroc_list), "auc std", np.std(auroc_list),
              "f1 mean", np.mean(f1_score_list), "f1 std", np.std(f1_score_list),
              "precision mean", np.mean(precision_list), "precision std", np.std(precision_list),
              "recall mean", np.mean(recall_list), "recall std", np.std(recall_list),
              "ap mean", np.mean(ap_list), "acc ap", np.std(ap_list),
              )

        test_losses = {"acc mean": np.mean(acc), "acc std": np.std(acc),
              "auc mean":np.mean(auroc_list), "auc std": np.std(auroc_list),
              "f1 mean": np.mean(f1_score_list), "f1 std": np.std(f1_score_list),
              "precision mean": np.mean(precision_list), "precision std": np.std(precision_list),
              "recall mean": np.mean(recall_list), "recall std": np.std(recall_list),
        ...
        with tqdm.tqdm(initial=self.state['current_iter'], total=int(self.args.total_iter_per_epoch * self.args.total_epochs)) as pbar_train:
            ...
            while (self.state['current_iter'] < (self.args.total_epochs * self.args.total_iter_per_epoch)) and (self.args.evaluate_on_test_set_only == False):
                ...
                for train_sample_idx, train_sample in enumerate(
                    ...
                    if self.state['current_iter'] % 2 == 0:
                    ...
                    if self.state['current_iter'] % 2 == 0:
                        self.evaluated_test_set_using_the_best_models(top_n_models=self.max_models_to_save, test_name="test1")
                        self.evaluated_test_set_using_the_best_models(top_n_models=self.max_models_to_save, test_name="test2")

    def test_set_using_the_best_models(self, top_models_idx, top_n_models, test_name="test1"):
        print(test_name, "  start  ", "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")

        top_n_idx = top_models_idx
        print(top_n_idx)
        per_model_per_batch_preds = [[] for i in range(top_n_models)]
        per_model_per_batch_targets = [[] for i in range(top_n_models)]
        test_losses = [dict() for i in range(top_n_models)]
        ...
        for i in range(top_n_models):
            ...
            recall_list.append(recall)
            ap_list.append(ap)
        print("test", "acc mean", np.mean(acc), "acc std", np.std(acc),
              "auc mean", np.mean(auroc_list), "auc std", np.std(auroc_list),
              "f1 mean", np.mean(f1_score_list), "f1 std", np.std(f1_score_list),
              "precision mean", np.mean(precision_list), "precision std", np.std(precision_list),
              "recall mean", np.mean(recall_list), "recall std", np.std(recall_list),
    ...
    def run_test(self, top_models_idx):
        self.test_set_using_the_best_models(top_models_idx, top_n_models=self.max_models_to_save, test_name="test1")
        self.test_set_using_the_best_models(top_models_idx, top_n_models=self.max_models_to_save, test_name="test2")
